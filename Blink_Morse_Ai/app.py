...
def main():
    ...
    while True:
        ...
        if current_state == CALIBRATION:
            ...
            if time_since_last >= WORD_PAUSE_THRESHOLD:
                ...
                if decoder.current_sequence:
                     ...
                     if word:
                         ...
                         tts.speak(word)
                         # Ensure audio has time to start playing
                         time.sleep(0.3)
                         decoder.complete_word() # Flush buffer
                     else:
                         # Invalid sequence, still flush to reset
                         print("[DEBUG] Invalid sequence, resetting")
                         decoder.reset()
            ...
            if not blinking and gap_duration >= WORD_PAUSE_THRESHOLD:
                ...
                if decoder.current_word:
                    ...
                    if word:
                        print(f"Morse Word Complete: {word}")
                        print(f"[TTS] Speaking: {word}")
                        tts.speak(word)
                        # Ensure audio has time to start playing
                        time.sleep(0.3)
        ...
        # Sufficient delay to allow TTS audio thread to process and play
        # 30ms gives smooth video (33 FPS) while allowing audio to play
